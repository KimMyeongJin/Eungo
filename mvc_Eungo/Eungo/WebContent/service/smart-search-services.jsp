										...
										<c:choose>
											...
											<c:otherwise>
												<c:forEach var="paging" items="${paging }">
													<li><a
														href="<%=request.getContextPath()%>/board?cmd=smart_search&pageNum=${paging.value}&per_page=${per_page}&search_text=${search_text}&search_category=${search_category}">${paging.key}</a></li>
												</c:forEach>
											</c:otherwise>
										</c:choose>
									</ul>
