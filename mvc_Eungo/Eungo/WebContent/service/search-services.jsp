									...
									<ul>
										<c:choose>
											<c:when test="${paging eq null }">
												<li><a
													href="<%=request.getContextPath()%>/board?cmd=board_paging&pageNum=1&per_page=9">해당
														게시물이 없습니다</a></li>
											</c:when>
											<c:otherwise>
												<c:forEach var="paging" items="${paging }">
													<li><a
														href="<%=request.getContextPath()%>/board?cmd=board_search&pageNum=${paging.value}&per_page=${per_page}&search_text=${search_text}">${paging.key}</a></li>
												</c:forEach>
											</c:otherwise>
										</c:choose>
									</ul>
...
function per_page() {
    ...
    location.href = "<%=request.getContextPath()%>/board?cmd=board_search&search_text=${search_text}&pageNum=1&per_page="+ x;
